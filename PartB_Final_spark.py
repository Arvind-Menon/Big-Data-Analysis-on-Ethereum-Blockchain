...
partbjob1output_join.saveAsTextFile("/user/arm34/PartB_Job1")

contracts = sc.textFile("/data/ethereum/contracts")
contracts_f = contracts.filter(clean_contracts)
contracts_join = contracts_f.map(lambda f: (f.split(',')[0],f.split(',')[4]))

partbjob2output = partbjob1output_join.join(contracts_join)

partbjob2output.saveAsTextFile("/user/arm34/PartA_Job2")
